...
#ifdef _WIN32
  // This makes a ninja run on a std::set of 1500 manifest files about 4% faster
...
int GetProcessorCount() {
  ...
  // processors threads can run on. This happens when a CPU std::set limitation
  // is active, see https://github.com/ninja-build/ninja/issues/1278
  cpu_set_t set;
...
#endif
  // Both truncate() and _chsize() return 0 on success and std::set errno and
  // return -1 on failure.
  if (success < 0) {
    *err = strerror(errno);
    return false;
