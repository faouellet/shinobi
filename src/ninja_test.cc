...
// This can't be a std::vector because tests call RegisterTest from static
// initializers and the order static initializers run it isn't specified. So
// the std::vector constructor isn't guaranteed to run before all of the
// RegisterTest() calls.
...
void Usage() {
  fprintf(stderr,
          ...
          "pattern.\n"
          "      '*' matches any substd::string. (gtest's ':', '?' are not "
          "implemented).\n");
}
