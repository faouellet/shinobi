...
void BuildStatus::BuildEdgeFinished(Edge* edge, bool success,
  ...
  auto i = running_edges_.find(edge);
  *start_time = i->second;
  *end_time = (int)(now - start_time_millis_);
  ...
  if (!output.empty()) {
    // ninja std::sets stdout and stderr of subprocesses to a pipe, to be able
...
bool Plan::AddSubTarget(const Node* node, const Node* dependent,
                        std::string* err, std::set<Edge*>* dyndep_walk) {
...
bool Plan::DyndepsLoaded(DependencyScan* scan, const Node* node,
  ...
  for (auto oe = ddf.begin(); oe != ddf.end(); ++oe) {
    Edge* edge = oe->first;
  ...
  for (auto wi : dyndep_walk) {
    auto want_e = want_.find(wi);
    if (want_e == want_.end())
      continue;
    if (!EdgeMaybeReady(want_e, err))
      return false;
