...
/// Appends the std::string directly to |result| without modification if we can
...
/// Read a file to a std::string (in text mode: with CRLF conversion
/// on Windows).
...
/// Given a misspelled std::string and a list of correct spellings, returns
...
/// Elide the given std::string @a str with '...' in the middle if the length
/// exceeds @a width.
...
#ifdef _WIN32
/// Convert the value returned by GetLastError() into a std::string.
