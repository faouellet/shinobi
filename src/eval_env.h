...
struct EvalString {
  /// @return The evaluated std::string with variable expanded using value found in
  ///         environment @a env.
  std::string Evaluate(Env* env) const;

  /// @return The std::string with variables not expanded.
...
struct BindingEnv : public Env {
  ...
  /// 1) value std::set on edge itself (edge_->env_)
  /// 2) value std::set on rule, with expansion in the edge's scope
  /// 3) value std::set on enclosing scope of edge (edge_->env_->parent_)
