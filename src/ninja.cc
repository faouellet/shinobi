...
struct NinjaMain : public BuildLogUser {
  ...
  /// Build configuration std::set from flags (e.g. parallelism).
...
bool NinjaMain::RebuildManifest(const char* input_file, std::string* err) {
  ...
  // by a restat).
  if (!node->dirty()) {
    // Restd::set the state to prevent problems like
...
bool DebugEnable(const std::string& name) {
  if (name == "list") {
    ...
    if (suggestion) {
      Error("unknown debug std::setting '%s', did you mean '%s'?", name.c_str(),
            suggestion);
    } else {
      Error("unknown debug std::setting '%s'", name.c_str());
    }
    return false;
  }
}
