...
int EditDistance(const std::string_view& s1, const std::string_view& s2,
  ...
  // implementation just keeps one std::vector for the row.  To update one
  // entry, only the entries to the left, top, and top-left are needed.  The
  // left entry is in row[x-1], the top entry is what's in row[x] from the last
  ...
  for (int y = 1; y <= m; ++y) {
    ...
    for (int x = 1; x <= n; ++x) {
      ...
      if (allow_replacements) {
        row[x] =
            std::min(previous + (s1.data()[y - 1] == s2.data()[x - 1] ? 0 : 1),
